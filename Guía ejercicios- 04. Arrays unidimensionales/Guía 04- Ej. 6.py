...
'''
6.	Escribir una función que reciba como parámetros una lista
de enteros y retorne la posición del valor máximo encontrado.
'''
...
def determinar_posicion_maximo(lista_para_calcular: list) -> int:
    ...
    for i in range(longitud):
        if (valor_maximo == None) or (lista_para_calcular[i] > valor_maximo):
            valor_maximo = lista_para_calcular[i]
            posicion_maximo = i
        
    return posicion_maximo

posicion_maximo_lista = determinar_posicion_maximo(lista_numeros_enteros)

print(f"El valor máximo de la lista está en la posición {posicion_maximo_lista}")
